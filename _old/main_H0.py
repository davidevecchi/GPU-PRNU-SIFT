...
def parallel_PCE(CXC, idx, ranges, squaresize=11):
    ...
    for i in range(0, idx):
        ...
        C_without_peak = _RemoveNeighborhood(
            C,
        ...
        for video_path in videos_path:
            ...
            if not os.path.exists(out_file):
                ...
                if orientation == 90 or orientation == 270:
                    ...
                    time_array = []
                    XC = (crosscorr_Fingeprint_GPU(
                        (tf.expand_dims([W_T1, W_T2], axis=3)), TA_tf, norm2,
                    ...
                    for idx_frame in set_idx_frame:
                        ...
                        if ret:
                            ...
                            XC = (crosscorr_Fingeprint_GPU(
                                (tf.expand_dims(tf.expand_dims(W_T, axis=0), axis=3)), TA_tf, norm2,
                                (1,) + np.shape(tilted_array2)
                                ))
                                    ...
                                    if no_of_matches > 4:
                                        ...
                                        if hom is not None and no_of_matches > 50:
                                            ...
                                            W_corr = tfa.image.transform(
                                                tf.convert_to_tensor(noisecorr, dtype=tf.float32),
                                                [1, 0, 0, 0, 1, 0, 0, 0], 'BILINEAR', [size_fing[0], size_fing[1]]
                                                )
                                            XC = (crosscorr_Fingeprint_GPU(
                                                (tf.expand_dims(tf.expand_dims(W_corr, axis=0), axis=3)), TA_tf,
                                            ...
                                            if pcecorr > pceres:
                                                start = time.time()
                                                homography, pce, rotation, scaling = calibration_GPU(
                                                    np.linalg.inv(hom), noise, 0, 0, 0, TA_tf, norm2, np.shape(TA_tf), [1, 0, 0, 0, 1, 0, 0, 0], pcecorr
                                                    )
                                                ...
                                                oframe = tfa.image.transform(
                                                    ...
                                                    'BILINEAR', [np.uint32(np.rint(resized.shape[0] / homography[0])),
                                                                 np.uint32(np.rint(resized.shape[1] / homography[0]))]
                                                    ).numpy()
                                            else:
                                                start = time.time()
                                                homography, pce, rotation, scaling = calibration_GPU(
                                                    np.zeros((3, 3)), noise, 0, 0, 0, TA_tf, norm2, (1,) + np.shape(tilted_array2), [1, 0, 0, 0, 1, 0, 0, 0], pceres
                                                    )
                                                pce_array.append(pce)
                                                time_array.append(time.time() - start_run)
                                                print('time calibration: ', time.time() - start)
